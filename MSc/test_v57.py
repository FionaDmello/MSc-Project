...
Lx = np.loadtxt("1X1.txt", unpack = True)	#Loading information from the end of the previous run as initial conditions for the current run.
Lv = np.loadtxt("1V1.txt", unpack = True)
Ndis0 = np.loadtxt("1Ndis0.txt", unpack = True)
d = np.loadtxt("1d.txt", unpack = True)
...
if __name__ == "__main__":
	...
	np.savetxt("1X2.txt",Xn)
	np.savetxt("1V2.txt",Vn)
