...
#Defining constants
pi = 3.1415926
n = 1501								#Total no.of particles.																	
rbar = 34.61 * (10**24)				# in meters 																	
M = 397.2 * (10**45)
l = 0.12				#Mass carried by each mass-element.					 
...
np.savetxt("1501Ndis0.txt",Ndis0)
np.savetxt("1501d.txt",d)
...
if __name__ == "__main__":
	...
	np.savetxt("1501X1.txt",X)					#Saving information about the system at 10 MY, to be used as initial conditions for the next run.
	np.savetxt("1501V1.txt",V)
	end = time.time()
	print end - start
	...
	plt.figure()
	for i in xrange(500,1001):	
		plt.plot(X[i],ind[i],'r.')
		plt.xlabel('Position (Mpc)')('Index i(from 500-1001)')
